...
## Android-review final polish

- Mobile account section label is Settings; page title and dropdown retain Workspace settings. Menu is visually smaller (13px type, reduced padding) with a 44px minimum target. Desktop remains avatar + chevron; the existing 1100px mobile/tablet breakpoint is preserved.
- Pursuit Sync opportunities moved inside the hero as a compact secondary action. Empty profiles now show Company profile not configured with Set up profile →; capabilities-only, company-only and complete profiles render natural copy without an empty colon. User data is rendered with textContent, not HTML interpolation; no profile validation/scoring/API behavior changed.
- Edit company profile → and View plan & billing → are consistent, non-underlined, focus-visible product actions with 44px minimum targets.
- Local Node 22 full gate: **124/124 tests PASS**, all existing smokes PASS, secret scan **0 findings**, diff check clean.
- Browser QA: **64 page/viewport combinations** at **360, 390, 430, 768, 980, 1024, 1100, 1440**, plus **32 profile-state checks** (empty, capabilities-only, company-only, complete at every width). No horizontal overflow or uncaught page errors. Existing keyboard/menu, avatar-only, capacity/import/archive/restore and password-confirmation checks still pass. Profile variants are local browser response fixtures, not writes to production.
- Screenshots and results: task workspace `work/rc20-android-qa/`, including `{width}-pursuit-{empty,capabilities,name,complete}.png`, `{width}-settings-viewport.png`, shell open/closed and the full-page set. Mobile settings, empty-profile Pursuit, tablet capabilities and desktop complete-profile captures were visually reviewed. Physical-device acceptance remains with the Android reviewer.
- Existing branch and PR #4 only; remote CI results are recorded on the PR after push. No automatic merge and no Railway, DNS, Stripe Live or production-data changes.
