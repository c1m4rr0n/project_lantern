...
function memoryStore(initial=null){let state=initial,events=[],analytics={milestones:{},events:[]};return{async getAnalytics(){return structuredClone(analytics);},async saveAnalytics(value){analytics=value;},async getAuditEvents(){return events;},async saveAuditEvents(next){events=next;},async getBilling(){return state?structuredClone(state):null;},async saveBilling(next){state=structuredClone(next);return next;},snapshot(){return state;}};}
