...
test('manual freshness persists across service instances and budget exhaustion preserves feed/summary',async t=>{
  ...
  let calls=0,clock=Date.now();const {provider,watchProvider,detailProvider}=createProviders({dataRoot:root,env:{DATA_PROVIDER:'sam',SAM_POST_DISCOVERY_QUIET_MS:'0',SAM_API_KEY:'fake',SAM_DAILY_REQUEST_BUDGET:'3',DISCOVERY_TARGET_RELEVANT:'1',DISCOVERY_TARGET_STRONG:'1',DISCOVERY_FRESHNESS_MS:'1000',DISCOVERY_REQUEST_INTERVAL_MS:'0'},fetchImpl:async url=>{
