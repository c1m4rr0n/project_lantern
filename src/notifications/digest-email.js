...
export function renderDigestEmail(digest) {
  ...
  const vendorAlerts=list(vendor.alerts,item=>`<li><strong>${esc(item.legalName||'Unnamed vendor')}</strong> — <strong style="color:${item.status==='excluded'?'#b42318':'#b54708'}">${esc(String(item.status||'unknown').toUpperCase())}</strong><br><span style="color:#475467">${esc(item.reason||'Screening status changed')}</span></li>`,'No unread vendor eligibility alerts.');
  const vendorHtml=vendor.total?`<h2>Vendor eligibility watch</h2><p style="color:#667085">${esc(vendor.total)} watched · ${esc(vendor.excludedCount||0)} excluded · ${esc(vendor.possibleMatchCount||0)} possible match · ${esc(vendor.alertCount||0)} unread alert${vendor.alertCount===1?'':'s'}</p>${vendorAlerts}`:'';
  ...
  const text=[
    ...
    ...(vendor.total?['','VENDOR ELIGIBILITY WATCH',`${vendor.excludedCount||0} excluded · ${vendor.possibleMatchCount||0} possible match · ${vendor.alertCount||0} unread alert(s)`,...(vendor.alerts?.length?vendor.alerts.map(x=>`- ${x.legalName||'Unnamed vendor'} — ${String(x.status||'unknown').toUpperCase()} — ${x.reason||'screening status changed'}`):['- No unread vendor eligibility alerts.'])]:[]),
    '',
