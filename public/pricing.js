...
function render(){
  ...
  const cancellation=state.cancelAtPeriodEnd&&state.currentPeriodEnd?` · cancels ${new Date(state.currentPeriodEnd).toLocaleDateString()}`:'';const label=state.plan==='trial'?`Trial · ${ent.trialDaysRemaining??0} day(s) remaining`:`${state.plan==='team'?'Scale':state.plan} · ${state.status}${cancellation}`;
