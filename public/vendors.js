...
async function removeVendor(id){const vendor=vendors.find(v=>v.id===id);if(!confirm(`Archive ${vendor?.legalName||'this vendor'} and stop monitoring it? Screening history will be preserved.`))return;try{await api(`/api/vendors/${encodeURIComponent(id)}`,{method:'DELETE'});setActionStatus('Vendor archived.');await refreshAfterSave();}catch(e){setActionStatus(`Could not archive vendor. ${friendlyMessage(e)}`,true);}}
